// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'task_tag_usecase_providers.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

String _$addTagToTaskUseCaseHash() =>
    r'561e769d103c5121d13afafba9fc1e6729247ba1';

/// See also [addTagToTaskUseCase].
@ProviderFor(addTagToTaskUseCase)
final addTagToTaskUseCaseProvider =
    AutoDisposeProvider<AddTagToTaskUseCase>.internal(
      addTagToTaskUseCase,
      name: r'addTagToTaskUseCaseProvider',
      debugGetCreateSourceHash:
          const bool.fromEnvironment('dart.vm.product')
              ? null
              : _$addTagToTaskUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef AddTagToTaskUseCaseRef = AutoDisposeProviderRef<AddTagToTaskUseCase>;
String _$getTagsForTaskUseCaseHash() =>
    r'02bb677530c266c7b50c55fc6fd6b9f21b884a7c';

/// See also [getTagsForTaskUseCase].
@ProviderFor(getTagsForTaskUseCase)
final getTagsForTaskUseCaseProvider =
    AutoDisposeProvider<GetTagsForTaskUseCase>.internal(
      getTagsForTaskUseCase,
      name: r'getTagsForTaskUseCaseProvider',
      debugGetCreateSourceHash:
          const bool.fromEnvironment('dart.vm.product')
              ? null
              : _$getTagsForTaskUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef GetTagsForTaskUseCaseRef =
    AutoDisposeProviderRef<GetTagsForTaskUseCase>;
String _$getTaskWithTagUseCaseHash() =>
    r'1854a96c149cbd581d4868fbc12e3bcc47763022';

/// See also [getTaskWithTagUseCase].
@ProviderFor(getTaskWithTagUseCase)
final getTaskWithTagUseCaseProvider =
    AutoDisposeProvider<GetTaskWithTagUseCase>.internal(
      getTaskWithTagUseCase,
      name: r'getTaskWithTagUseCaseProvider',
      debugGetCreateSourceHash:
          const bool.fromEnvironment('dart.vm.product')
              ? null
              : _$getTaskWithTagUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef GetTaskWithTagUseCaseRef =
    AutoDisposeProviderRef<GetTaskWithTagUseCase>;
String _$removeTagFromTaskUseCaseHash() =>
    r'b7042837f865bc291f27ef803139eed6dce9325a';

/// See also [removeTagFromTaskUseCase].
@ProviderFor(removeTagFromTaskUseCase)
final removeTagFromTaskUseCaseProvider =
    AutoDisposeProvider<RemoveTagFromTaskUseCase>.internal(
      removeTagFromTaskUseCase,
      name: r'removeTagFromTaskUseCaseProvider',
      debugGetCreateSourceHash:
          const bool.fromEnvironment('dart.vm.product')
              ? null
              : _$removeTagFromTaskUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef RemoveTagFromTaskUseCaseRef =
    AutoDisposeProviderRef<RemoveTagFromTaskUseCase>;
String _$removeAllTagsFromTaskUseCaseHash() =>
    r'28b808f6d0a7872f6e6a73c7988599fc6dd8c1f8';

/// See also [removeAllTagsFromTaskUseCase].
@ProviderFor(removeAllTagsFromTaskUseCase)
final removeAllTagsFromTaskUseCaseProvider =
    AutoDisposeProvider<RemoveAllTagsFromTaskUseCase>.internal(
      removeAllTagsFromTaskUseCase,
      name: r'removeAllTagsFromTaskUseCaseProvider',
      debugGetCreateSourceHash:
          const bool.fromEnvironment('dart.vm.product')
              ? null
              : _$removeAllTagsFromTaskUseCaseHash,
      dependencies: null,
      allTransitiveDependencies: null,
    );

@Deprecated('Will be removed in 3.0. Use Ref instead')
// ignore: unused_element
typedef RemoveAllTagsFromTaskUseCaseRef =
    AutoDisposeProviderRef<RemoveAllTagsFromTaskUseCase>;
// ignore_for_file: type=lint
// ignore_for_file: subtype_of_sealed_class, invalid_use_of_internal_member, invalid_use_of_visible_for_testing_member, deprecated_member_use_from_same_package
